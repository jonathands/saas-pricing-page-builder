...
export function ThemedPricingPage({
  ...
  const primaryHslValue = primaryHsl
    ? `${primaryHsl[1]} ${primaryHsl[2]}% ${primaryHsl[3]}%`
    : "262 80% 50%";

  const secondaryHsl = theme.secondaryColor.match(
    /hsl\((\d+),\s*(\d+)%,\s*(\d+)%\)/
  );
  const secondaryHslValue = secondaryHsl
    ? `${secondaryHsl[1]} ${secondaryHsl[2]}% ${secondaryHsl[3]}%`
    : "220 14.3% 95.9%";
    ...
    >
      <style
        dangerouslySetInnerHTML={{
          ...
          .themed-pricing-page {
            --tw-gradient-from: ${
              theme.backgroundColor.includes("from-")
                ? `var(--${theme.backgroundColor.split("from-")[1].split(" ")[0]})`
                : "rgb(248 250 252)"
            };
            --tw-gradient-to: ${
              theme.backgroundColor.includes("to-")
