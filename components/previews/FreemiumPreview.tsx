...
import { FreemiumModel } from "@shared/pricing";

interface FreemiumPreviewProps {
  strategy: FreemiumModel;
}
...
export function FreemiumPreview({ strategy }: FreemiumPreviewProps) {
  ...
  return (
    <div
      className={`grid gap-4 lg:gap-6 ${allTiers.length === 2 ? "grid-cols-1 lg:grid-cols-2" : allTiers.length === 3 ? "grid-cols-1 md:grid-cols-2 xl:grid-cols-3" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3"}`}
    >
      {allTiers.map((tier) => (
        <Card
          key={tier.id}
        ...
        >
          ...
          <CardHeader className="text-center">
            <CardTitle className={tier.isFree ? "text-green-600" : ""}>
              {tier.name}
            </CardTitle>
            <div className="space-y-1">
              <div
                className={`text-3xl font-bold ${tier.isFree ? "text-green-600" : ""}`}
              >
