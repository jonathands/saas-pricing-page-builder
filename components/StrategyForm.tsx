...
import { Plus, Trash2, Package, Settings, Zap, CreditCard } from "lucide-react";
import {
  PricingModel,
  FlatRateModel,
  TieredModel,
...
export function ModelForm({ model, onUpdate }: ModelFormProps) {
  ...
  const updateNestedField = (
    ...
    const parent = (model as any)[parentField];
    onUpdate(model.id, {
      [parentField]: { ...parent, [field]: value },
    });
  };

  if (model.type === "flat-rate") {
    const flatModel = model as FlatRateModel;
    return (
  ...
  if (model.type === "tiered") {
    const tieredModel = model as TieredModel;
    return (
      <div className="space-y-4">
        <div className="text-sm font-medium text-slate-200">
          Tiers Configuration
        </div>
        {tieredModel.tiers.map((tier, index) => (
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input
                    value={tier.name}
                    onChange={(e) => {
                      const newTiers = [...tieredModel.tiers];
  ...
  if (model.type === "usage-based") {
    const usageModel = model as UsageBasedModel;
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Input
              type="number"
              value={usageModel.includedUsage}
              onChange={(e) =>
  ...
  if (model.type === "per-user") {
    const userModel = model as PerUserModel;
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
  ...
  if (model.type === "feature-based") {
    const featureModel = model as FeatureBasedModel;
    ...
    const updateFeature = (index: number, field: string, value: any) => {
      const newFeatures = [...featureModel.features];
      newFeatures[index] = { ...newFeatures[index], [field]: value };
      updateField("features", newFeatures);
    };

    const removeFeature = (index: number) => {
      const newFeatures = featureModel.features.filter(
        (_, i) => i !== index
      );
      updateField("features", newFeatures);
    };
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <Label className="text-slate-200">Billing Period</Label>
            <Select
              value={featureModel.billingPeriod}
              onValueChange={(value) => updateField("billingPeriod", value)}
            >
              <SelectTrigger className="bg-slate-700 border-slate-600 text-slate-100">
                <SelectValue />
        ...
        <div className="space-y-3">
          ...
          {featureModel.features.map((feature, index) => (
            <Card
              key={feature.id}
          ...
          {featureModel.features.length === 0 && (
  ...
  if (model.type === "freemium") {
    const freemiumModel = model as FreemiumModel;

    const addPaidTier = () => {
      const newPaidTiers = [
        ...freemiumModel.paidTiers,
        {
          id: `tier-${Date.now()}`,
          name: "New Tier",
          price: 29,
    ...
    const updatePaidTier = (index: number, field: string, value: any) => {
      const newPaidTiers = [...freemiumModel.paidTiers];
      newPaidTiers[index] = { ...newPaidTiers[index], [field]: value };
      updateField("paidTiers", newPaidTiers);
    };

    const removePaidTier = (index: number) => {
      const newPaidTiers = freemiumModel.paidTiers.filter(
        (_, i) => i !== index
      );
      updateField("paidTiers", newPaidTiers);
    };
      ...
      <div className="space-y-6">
        ...
        <Card className="p-4 bg-green-900/20 border-green-700">
          <div className="space-y-4">
            ...
            <FeatureEditor
              features={freemiumModel.freeTier.features}
              onChange={(features: EnhancedFeature[]) => {
                const newFreeTier = { ...freemiumModel.freeTier, features };
            ...
            <div>
              ...
              <Input
                type="number"
                value={freemiumModel.freeTier.usageLimit || ""}
                onChange={(e) => {
                  const newFreeTier = {
                    ...freemiumModel.freeTier,
        ...
        <div className="space-y-4">
          ...
          {freemiumModel.paidTiers.map((tier, index) => (
            <Card key={tier.id} className="p-4 bg-slate-700 border-slate-600">
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <FeatureEditor
                  features={tier.features}
                  onChange={(features: EnhancedFeature[]) =>
                    updatePaidTier(index, "features", features)
                  }
                />
              </div>
            </Card>
          ...
          {freemiumModel.paidTiers.length === 0 && (
            <div className="text-center py-8 bg-slate-800 rounded-lg border-2 border-dashed border-slate-600">
  ...
  return (
    <div className="p-4 text-center text-slate-400">
      Configuration form for {(model as any).type} coming soon...
    </div>
  );
}
