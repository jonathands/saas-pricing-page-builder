...
export function ComparisonTable({
  ...
  strategies: PricingModel[];
}) {
  const getStrategyMetrics = (strategy: PricingModel) => {
    switch (strategy.type) {
      case "flat-rate":
        const flatModel = strategy as FlatRateModel;
        return {
          startingPrice: flatModel.price,
          scalability: "Low",
          complexity: "Very Low",
          customerType: "SMB",
      ...
      case "usage-based":
        const usageModel = strategy as UsageBasedModel;
        return {
          startingPrice: usageModel.basePrice,
      ...
      case "tiered":
        const tieredModel = strategy as TieredModel;
        return {
          startingPrice: Math.min(...tieredModel.tiers.map((t) => t.price)),
          scalability: "High",
          complexity: "Medium",
      ...
      case "per-user":
        const userModel = strategy as PerUserModel;
        return {
          startingPrice: userModel.pricePerUser * userModel.minimumUsers,
          scalability: "High",
          complexity: "Low",
          customerType: "Teams",
...
export function CostChart({ strategies }: { strategies: PricingModel[] }) {
  ...
  const chartData = usagePoints.map((usage) => {
    ...
    strategies.forEach((strategy) => {
      ...
      switch (strategy.type) {
        case "flat-rate":
          cost = (strategy as FlatRateModel).price;
          break;
        case "usage-based":
          const usageModel = strategy as UsageBasedModel;
          const overage = Math.max(0, usage - usageModel.includedUsage);
          cost = usageModel.basePrice + overage * usageModel.usagePrice;
          break;
        case "per-user":
          // Assume 5 users for comparison
          const userModel = strategy as PerUserModel;
          cost =
            Math.max(5, userModel.minimumUsers) * userModel.pricePerUser;
          break;
        case "tiered":
          // Use middle tier for comparison
          const tieredModel = strategy as TieredModel;
          const middleTier =
            tieredModel.tiers[Math.floor(tieredModel.tiers.length / 2)];
